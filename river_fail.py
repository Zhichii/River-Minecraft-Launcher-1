    ...
    while 1:
        ...
        for i in range(selectedWrapper, selectedWrapper + settings["downloadRenderDistance"]["value"]):
            ...
            if (i == selectedWrapper):
                ...
                print("  ", end = "")
            print(lang["optifine.forge"].replace("%1", wrappers[i][1]).replace("%2", wrappers[i][3]), end = "")
            if (i == selectedWrapper):
                print("] ")
            else:
                print("  ")
        ...
        if (press == b' '):
            ...
            web = get(wrapper[2]).decode("utf-8").splitlines()
            x = ""
            for i in web:
    ...
    while 1:
        ...
        if (press == b'w'):
            if ((selectedLaunch-1) >= 0):
                selectedLaunch -= 1
        if (press == b's'):
            if ((selectedLaunch+1) < (len(launch))):
                selectedLaunch += 1
        if (press == b'd'):
            if (launch == []):
                print(lang["launch.instead"])
